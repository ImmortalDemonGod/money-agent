...
assert_exit 0 "decision gate (P3): pinned provenance manifest passes" \
  python3 bin/decision_gate.py data-acquisition dg_acq2.txt
# S16 C5 bite test: a NAMED manifest whose hash does NOT match the declared provenance blocks --
# the pin is to a specific file's committed bytes, not "any file sharing the hash".
replace_once DECISION_LOG.md "provenance:$PROV" "provenance:$(python3 -c "print('a'*64)")"
assert_exit_grep 1 "not" "decision gate (P3, S16 C5): named manifest whose hash mismatches the pin blocks" \
  python3 bin/decision_gate.py data-acquisition dg_acq2.txt
