...
**Handling note (leak-check):** This document contains STRATEGIC content — segment selection, product classes, channel intersections (§7 Wall 5, §9 Issues 3–5). Under the repo's own leak-check rule (`knowledge/README.md`, V2_DESIGN M9), none of it may enter `knowledge/`, `templates/`, PROMPT.md, or any surface the run agent reads, or the context-free premise breaks and the next run's convergence stops meaning anything. It lives in `docs/` (explicitly excluded from the run agent's inputs by CLAUDE.md). Harness-side items ([H]-tagged in §9) are safe to implement mechanically; the *reasoning* behind them stays here.
...
**The integrity constraint over all of the above:** these issues split into harness-side (operator's to fix mechanically: 1, 2, 7, and the provisioning halves of 3–4) and strategy-side (the agent's to rediscover in-run: the substance of 3, 4, 5). Writing strategy-side conclusions into `knowledge/` or the prompt would break the context-free premise that makes the agent's convergence meaningful — the repo's own leak-check rule. The clean move: fix the harness, then let the next run meet a properly instrumented, properly time-scaled world.
...
Five components (R1–R5). Each is specified against the files and mechanisms that exist today; P-numbers refer to `V2_HARNESS_DESIGN.md` §15.2. All are [H] harness-side and strategy-free: the harness provides capability; whether and where to use it stays the agent's in-run decision (leak-check preserved).
